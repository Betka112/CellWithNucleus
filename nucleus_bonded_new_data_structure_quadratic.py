...
from espressomd.interactions import QuarticBond
...
sim_no = sys.argv[21]
# quadratic bond parameter
par_k0 = float(sys.argv[22])
angle_par = float(sys.argv[23])
par_k1 = float(sys.argv[24])
# parameters of the constriction
par_ax = float(sys.argv[25])
par_ay = float(sys.argv[26])
par_az = float(sys.argv[27])
...
for p in nucleus.mesh.points:
    ...
    for pMembrane in membrane.mesh.points:
        ...
        if angle < small_angle:
            quarticInter = QuarticBond(k0=par_k0, k1=par_k1, r=tmpDist, r_cut=30.0)
            system.bonded_inter.add(quarticInter)
            p.part.add_bond((quarticInter, pMembrane.part_id))
