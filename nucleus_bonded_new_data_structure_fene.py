...
for p in nucleus.mesh.points:
    ...
    for pMembrane in membrane.mesh.points:
        ...
        if angle < small_angle:
            feneInter = FeneBond(k=par_k, d_r_max=par_max, r_0=tmpDist)
            system.bonded_inter.add(feneInter)
            p.part.add_bond((feneInter, pMembrane.part_id))
